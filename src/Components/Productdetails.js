...
function Productdetails() {
  ...
  const getKeywordData = async () => {
    try {
      const response = await axios.get(
        "https://api.proleverageadmin.in/api/getallkeyword"
      );
  ...
  // useEffect(() => {
  //   const filtered = asindata.filter((i) => i.asin === data.asin);
  //   console.log("filtered====", filtered);

  //   if (filtered.length > 0 && productPrice) {
  //     const category = filtered[0]?.category;
  //     console.log("category inside useEffect====", category);

  //     const fee = calculateReferralFee(category, productPrice);
  //     console.log("calculated fee====", fee);

  //     setReferralFee(fee);
  //   }
  // }, [asindata, data, productPrice, referralFee]);
  ...
  const refereal = productPrice * 0.08;

  console.log("refereal", refereal);
      ...
      <div className="row" style={{ justifyContent: "center" }}>
        ...
        <p>Cost Per Unit: {costPerUnit}</p> */}
              ...
              >
                ...
                >
                  ...
                  >
                    Star Rating <br />
                    <span
                      className="poppins-regular mx-2 pt-1"
                      style={{
                        fontSize: "11px",
                      }}
                    >
                      ...
                      {Array.from({ length: 5 }).map((_, index) => (
                        <span
                          key={index}
                          className={
                            index < Math.round(data.product_star_rating)
                              ? "filled-star"
                              : "empty-star"
                          }
                        >
          ...
          >
            ...
            >
              Referral Fee - {Math.round(refereal)}
            </div>
          ...
          >
            ...
            >
              ...
              {Math.round(
                // productPrice +
                refereal + closingFee + (data.currency === "US" ? 2 : 80) // Assuming $1 = ₹83 for conversion
              )}
            </div>
          </div>
